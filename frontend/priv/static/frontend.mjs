...
function createElementNode({ prev, next, dispatch, stack }) {
  ...
  for (const attr of next.attrs) {
    ...
    if (attr.as_property) {
      ...
      if (el2.getAttribute(name2) !== value)
        el2.setAttribute(name2, value);
...
// build/dev/javascript/frontend/frontend/pages/login.mjs
function login_view() {
  return div(
    toList([]),
    toList([
      ...
      div(
        ...
        toList([
          ...
          input(
            toList([name("password"), type_("password")])
          )
        ])
      ),
      button(toList([]), toList([text("Submit")]))
    ])
  );
}
...
function init3(_) {
  return [new Login(), init2(on_url_change)];
}
